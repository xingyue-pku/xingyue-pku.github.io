---
layout: archive
title: "学术论文"
permalink: /publications/
author_profile: true
---

> `*` 为通讯作者。按时间倒序排列。

## 2026

- **Xing Y**, Huang W, Wang Y. Reconfiguring global LIS education: AI integration, pedagogical roles and epistemic challenges[J]. The Electronic Library, 2026: 1-23.

- 申佳骏, **邢玥\***. 科研信息活动对科研创新表现的影响研究——基于博士生过程数据的实证分析[J/OL]. 图书馆建设, 1-25[2026-06-02]. <https://link.cnki.net/urlid/23.1331.G2.20260415.0958.002>.

- 卢欣雨, 黄文彬, **邢玥\***. 基于大语言模型的学术文献自动摘要质量评价体系构建与实证研究[J]. 情报科学, 2026. 已接收, 待刊.

- **邢玥**. 外审分歧的文本形成机制研究: 基于同一稿件多评审文本的分析[C]. 中国情报学年会 2026 · 博士生学术论坛, 2026. 已录用 (海报).

- 顾舒文, **邢玥\***, 黄文彬, 解贺嘉, 冯菲. GenAI 介入高校科研实践的风险感知结构及治理启示[J]. 图书情报工作, 2026. 在审.

- 顾舒文, **邢玥\***, 黄文彬. 生成式人工智能介入科研的风险感知与治理研究——基于高校师生科研使用情境的质性分析[C]. 中国情报学年会 2026 · 博士生学术论坛, 2026. 已录用 (报告交流).

- **Xing Y**, Xie H, Gu S, Huang W. After the Artifact: Reconstructing Academic Legitimacy[C]. ASIS&T Annual Meeting 2026 (Poster), 2026. 投出 (在审).

- **Xing Y**, Wang Y, Liu Q, Huang W. Capturing Movement in Action: A Methodological Approach Using Classroom Monitoring Videos in Preschools[C]. IEEE ICCSE 2026, 2026. 投出 (在审).

- Wang Y, **Xing Y**, Huang W. Digital Literacy Research and Its Implications for Experimental Teaching in Higher Education: A Review[C]. IEEE ICCSE 2026, 2026. 已录用.

## 2025

- **邢玥**, 朱桃林, 黄文彬. 博士阶段课程与研究选题之间的关联性分析: 基于语义相似度的课程优化视角[J]. 教学研究, 2025, 48(4): 66-77.

- 王洋洋, 李悦天, **邢玥**, 黄文彬. 数字化转型视角下高校实验室管理的探索与实践[J]. 计算机科学, 2025, 52(10A). 增刊.

- 沈同, 黄文彬, **邢玥**. 基于数字化学习行为分析的小学生个性化学习模式研究——以云南省弥渡县新城完全小学为例[C]. 第三届粤港澳促进 STEAM 教育大会, 2025. 已录用.

## 2024

- Wang L, Zeng X, **Xing Y**. Inconsistencies between journal policies and editors in China[J]. Publishing Research, 2024, 3(1).

- **邢玥**, 王文博, 赵思琪. 从数据到决策: 基于可解释机器学习的博士研究生延期风险影响因素研究[C]. 第六届全国高校高等教育学研究生学术论坛, 2024.

- **Xing Y**, Wang W B, Zhu T L. Towards efficient doctoral programs: Predictive modeling of graduation timelines and delays[C]. GCCIL 2024, 2024.

---

## 编著

- 数智教育发展国际大学联盟指南研制课题组. 高等教育人工智能应用边界指南 2.0 (汉英对照)[M]. 北京: 北京出版集团 · 北京出版社, 2026. 参编.
